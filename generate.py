...
def parsing():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Cut off the number of images to process",
    )
    parser.add_argument(
        "--log-interval",
        "--log_interval",
...
def get_image_list(args):
    ...
    image_map = dict()
    for i in range(1000):
        image_map[i] = os.listdir(
            os.path.join(args.data_path, "val", key_map[holder[i]])
        )
...
def computeAndSaveMaps(args):
    ...
    for idx in pbar:
        ...
        for c, file in enumerate(os.listdir("data/imagenet/val/"+ classes[idx])):
            ...
            image = loadImage(
                os.path.join(args.data_path, "val", classes[idx], file),
            ...
            saliencyMap = batch_normalize_map(
                saliencyMap,
                image_size=(
                    (args.input_size,) * 2
                    if saliencyMap.shape[-1] != args.input_size
                    else None
                ),
            )
            ...
            saveMapWithColorMap(
                os.path.join(
                    ...
                    savedir,
                    "{}_{}_{}_{:.4f}.{}".format(
                        name, result, preds, orig_pred[0, idx].item(), ext
                    ),
                ),
                saliencyMap,
                rawImage,
